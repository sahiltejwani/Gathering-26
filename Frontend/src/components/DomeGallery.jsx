...
const DEFAULT_IMAGES = Array.from({ length: 28 }, (_, i) => ({
...
export default function DomeGallery({
  ...
  // inside DomeGallery, after other hooks but before return
useEffect(() => {
  const speed = 0.03; // degrees per frame, very slow
  let rafId;

  const step = () => {
    rotationRef.current = {
      x: rotationRef.current.x,
      y: wrapAngleSigned(rotationRef.current.y + speed),
    };
  ...
  useEffect(() => {
    const styleId = 'gallery-inline-style';
